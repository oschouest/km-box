...
use async_hid::{HidResult, DeviceInfo};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if args.list_devices {
        ...
        let devices = async_hid::enumerate().await?;
...
async fn monitor_mouse_device(vid: u16, pid: u16, tx: mpsc::Sender<InputEvent>) -> HidResult<()> {
    ...
    let mut device = async_hid::open(mouse_device.path()).await?;
...
async fn monitor_keyboard_device(tx: mpsc::Sender<InputEvent>) -> HidResult<()> {
    ...
    let mut device = async_hid::open(keyboard_device.path()).await?;
